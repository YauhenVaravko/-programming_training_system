...
public class Lesson2Controller implements Initializable {
    ...
    @FXML
    private ScrollPane les2_main_scroll_panel;

    @FXML
    private AnchorPane les2_main_panel;
    ...
    @FXML
    private Tab lesson2_tab;

    @FXML
    private TextArea code_text_area_lesson2;
    ...
    public Stage getStage() {
        if(this.stage==null)
            this.stage = (Stage) this.les2_main_scroll_panel.getScene().getWindow();
        return stage;
    }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        les2_main_scroll_panel.setVvalue(0);
        setDefaultTextArea();
    ...
    private void setDefaultTextArea() {
        ControllerMethods.setDefaultTextArea("./src/Lesson2/Lesson2Default.txt",code_text_area_lesson2);
    }
    private void reloadFile(){
        ControllerMethods.reloadFile("./src/Lesson2/Lesson2.java", code_text_area_lesson2);
    }
    private void runTask()  {
        ControllerMethods.runTask("./src/Lesson2/runTask.bat");
    }
    private void resultToConsole(){
        ControllerMethods.resultToConsole("./src/Lesson2/answer.txt",console_text_area);

    }
    private void checkAnswer(){
        console_text_area.clear();
        Map<String, String> checkList = new HashMap<String, String>();
        checkList.put("public class Lesson2",
                "- Имя класса должно быть Lesson2 и он должен быть публичным.");
        checkList.put("public static void main",
                "- Текст программы должен включать публичный статический метод main.");
        checkList.put("for",
                "- Необходимо использовать 2 цикла for");
        Map<String, Boolean> boolCheckList = new HashMap<String, Boolean>();
        boolCheckList.put("public class Lesson2",false);
        boolCheckList.put("public static void main",false);
        boolCheckList.put("for",false);
        ControllerMethods.searchForMatches(boolCheckList,"./src/Lesson2/Lesson2.java");
        boolean haveDifferent = false;
        haveDifferent=ControllerMethods.haveDifferent("./src/Lesson2/answer.txt","./src/Lesson2/correctAnswer.txt");
        if(haveDifferent){
            ...
            console_text_area.appendText("   - Программа должна выводить сообщение согласно заданию. (без ковычек и в качестве разделителя между символами использовать нижнее подчеркивание).\n");
            for (Map.Entry<String, String> element : checkList.entrySet()) {
            ...
            console_text_area.appendText("Выполненные требования : \n");
            console_text_area.appendText("  - Программа должна выводить сообщение согласно заданию. (без ковычек и в качестве разделителя между символами использовать пробел).\n");
            for (Map.Entry<String, String> element : checkList.entrySet()) {
                if(boolCheckList.get(element.getKey())==true){
                    console_text_area.appendText("   " + element.getValue() + "\n");
                }
